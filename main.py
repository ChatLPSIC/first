...
class main(MDApp):
    def __init__(self, **kwargs):
        ...
        # Thread
        self.recup_thread = True
        self.first_flag = True
        self.demander_flag = True

        # event schelude
        self.event_first = None
        self.event_demander = None
    ...
    def initial_server(self):
        ...
        self.recup_thread = False
        self.demander_flag = False

        s3 = self.root.get_screen('accepted')
        s4 = self.root.get_screen('chat')
        s5 = self.root.get_screen('connected')
        ...
        self.root.current = "choice"  # Passer à l'écran suivant
    ...
    def initial_client(self, user):
        ...
        if user in self.card_dic:
            del self.card_dic[user]
    ...
    def initial_conn(self):
        ...
        s5.check = False
        s5.verifier = False
        s5.ids.user_list.clear_widgets()
        s5.ids.request_container.clear_widgets()

        #   envoyer un message de deconnexion au serveur
        self.root.current = "connection"  # Passer à l'écran suivant


    def build(self):
        # Chargement des fichiers KV pour les différentes écrans
        Builder.load_file("classes_import.kv")
        Builder.load_file("choice.kv")
        Builder.load_file("activation.kv")
        Builder.load_file("connection.kv")
        Builder.load_file("accepted.kv")
        Builder.load_file("chat.kv")
        Builder.load_file("connected.kv")

        # Création et ajout des écrans au gestionnaire d'écrans
        sm = MDScreenManager()
        sm.add_widget(ServerClientChoiceScreen(name="choice")) # choice   screen_1
        sm.add_widget(ServerActivationScreen(name="activation")) # activation   screen_2_1
        sm.add_widget(ServerDetailsScreen(name="detail")) # detail    screen_2_1_1
        sm.add_widget(ClientConnectionScreen(name="connection")) # connection   screen_2_2
        sm.add_widget(QRCodeScanScreen(name="scann")) # Scann    scann
        sm.add_widget(ConnectedClientsScreen(name="connected")) #connected   screen_5
        sm.add_widget(AcceptedClientsScreen(name="accepted")) #accepted screen_3
        sm.add_widget(ChatScreen(name="chat")) #chat    screen_4
    ...
    def start_server(self):
        ...
        s2_1 = self.root.get_screen('activation')
        ...
        if all([self.pseudo_s.text.strip(), self.desc_s.text.strip(), self.port_s.text.strip(), self.n_s.text.strip()]):
            if not (
                ...
                if self.server_handler.running:
                    s2_1_1 = self.root.get_screen('detail')
                    ...
                    self.root.current = "detail"  # Passer à l'écran suivant
    ...
    def stop_server(self):
        # Arrêter le serveur
        # Envoye à un message à tous les clienct
    ...
    def first_thread(self, *args):
        ...
        def add_client(dt):
            if self.first_flag:
                ...
                Clock.unschedule(self.event_first)

        # Vérifie l'ajout de clients toutes les secondes
        def check_thread():
            self.event_first = Clock.schedule_interval(add_client, 1)
    ...
    def connect_client(self):
        ...
        s2_2 = self.root.get_screen('connection')
        ...
        if all([self.pseudo_c.text.strip(), self.desc_c.text.strip(), self.port_c.text.strip(),
            ...
            if not (self.error_pseudo or self.error_ip or self.error_port or self.error_desc):
                ...
                if self.client_handler.connected:
                    ...
                    self.root.get_screen('chat').start_listening()  # Démarrer l'écoute sur l'écran de chat
    ...
    def disconnect_client(self):
        ...
        if self.client_handler:
            ...
            self.initial_client(self.pseudo_c.text) # retrer le peudo des dictionnaire
            self.initial_conn() # initialiser les valeur de connexion

    def recup_aes(self):
        # Récupère les clés AES des utilisateurs connectés.
        while self.client_handler.connected:
            if self.recup_thread:
                ...
                if msg:
                    ...
                    self.client_handler.reponsa = None  # Réinitialise la réponse.
                else:
    ...
    def send_message(self):
        ...
        chat_screen = self.root.get_screen('chat')  # Accède à l'écran de chat.
    ...
    def demander(self):
        ...
        def check_demande(dt):
            if self.demander_flag:
                ...
                Clock.unschedule(self.event_demander)

        def demander_thread():
            self.event_demander = Clock.schedule_interval(check_demande, 1)  # Vérifie les demandes toutes les secondes.
    ...
    def add_user_request(self, user):
        ...
        accept_button = MDFillRoundFlatButton(
            ...
            size=("100dp", "40dp"),
            padding=("8dp", "4dp"),
            on_release=lambda x: self.accepter(user),  # Appelle la méthode pour accepter la demande.
        )
        ...
        refuse_button = MDFillRoundFlatButton(
            ...
            size=("100dp", "40dp"),
            padding=("8dp", "4dp"),
            on_release=lambda x: self.refuser(user),  # Appelle la méthode pour refuser la demande.
        )
        if user not in self.card_dic:
            self.card_dic[user] = user_card
            ...
            screenn5 = self.root.get_screen("connected")  # Accède à l'écran 5.
    ...
    def refuser(self, user):
        ...
        self.message_retour("refusé", user)  # Envoie une réponse de refus.
        screen5 = self.root.get_screen('connected')

        if user in self.card_dic:
            screen5.ids.request_container.remove_widget(self.card_dic[user])  # Retire la carte de la demande de l'interface.
            del self.card_dic[user]

    def accepter(self, user):
        screen5 = self.root.get_screen('connected')
        # Accepte une demande d'utilisateur et met à jour l'interface.
        if user not in screen5.user_accept:
            screen5.user_accept.append(user)  # Ajoute l'utilisateur à la liste des utilisateurs acceptés.
            self.message_retour("accepté", user)  # Envoie une réponse d'acceptation.

            # Supprime les boutons "Accepter" et "Refuser".
            if user in self.card_dic:
                user_card = self.card_dic[user]
                ...
                retirer_button = MDFillRoundFlatButton(
                    ...
                    size=("100dp", "40dp"),
                    padding=("8dp", "4dp"),
                    on_release=lambda x: self.retirer(user),  # Appelle la méthode pour retirer l'utilisateur.
                )
    ...
    def retirer(self, user):
        ...
        self.message_retour(f"retiré", user)  # Envoie un message de retrait.
        screen5 = self.root.get_screen('connected')
        ...
        self.initial_client(user)

    def message_retour(self, msg, user):
        # Envoie un message de retour au client connecté.
        msg_r = "reponse:" + user + ":" + msg + ":" + self.pseudo_c.text
