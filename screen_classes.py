...
class ServerClientChoiceScreen(MDScreen):
...
class ServerActivationScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        pass  # Pas de logique ajoutée pour l'instant
...
class ServerDetailsScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        pass  # Pas de logique ajoutée pour l'instant
...
class ClientConnectionScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        pass  # Pas de logique ajoutée pour l'instant
...
class QRCodeScanScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        pass  # Pas de logique ajoutée pour l'instant
...
class AcceptedClientsScreen(MDScreen):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.update = False
    ...
    def schedule_client_updates(self):
        self.event_update = Clock.schedule_interval(self.update_clients, 1)  # Mise à jour toutes les secondes
    ...
    def update_clients(self, dt=None):  # dt pour compatibilité avec Clock.schedule_interval
        if self.update:
            ...
            screen5 = app.root.get_screen('connected')  # Accéder à l'écran 5
            ...
            Clock.unschedule(self.event_update)
    ...
    def switch_to_chat_screen(self, user):
        app = MDApp.get_running_app()  # Obtenir l'application en cours d'exécution
        screen4 = app.root.get_screen('chat')  # Accéder à l'écran 4
        ...
        self.manager.current = "chat"  # Changer l'écran actuel
...
class ChatScreen(MDScreen):
    ...
    def recuperation(self):
        ...
        self.query = '''
            ...
            '''
        # Récupérer les messages de la base de données
        self.user = self.app.connected_user
    ...
    def schedule_message_check(self):
        ...
        self.event_listen = Clock.schedule_interval(self.listen_for_messages, 1)  # Vérification chaque seconde
    ...
    def listen_for_messages(self, dt=None):
        if self.listen:
            ...
            Clock.unschedule(self.event_listen)
        ...
        try:
            ...
            if len(separe) == 5:  # Vérifie que le message a 5 parties
                ...
                if statut == "non":  # Vérifie si le statut est "non"
                    ...
                    self.app.db.store_message(sender, "me", self.app.cry_status, msg)
                    if sender == self.user:
                        Clock.schedule_once(
...
class ConnectedClientsScreen(MDScreen):
    def __init__(self, **kwargs):
    ...
    def check_for_updates(self, *args):
        ...
        def check_for_clients(dt):
            if self.check:
                ...
                current_clients_s = app.client_handler.stop
                disconnected_client = app.client_handler.deconnexion
                if disconnected_client:
                    notification.notify(
                        title="Friend",
                        message=disconnected_client + " diconnected",  # Message de notification.
                        app_name='My Application',
                        app_icon=None,  # Optionnel : chemin vers une icône.
                        timeout=50  # Durée avant la disparition de la notification.
                    )
                    app.retirer(disconnected_client)
                    app.initial_client(disconnected_client)
                    app.client_handler.deconnexion = None

                if current_clients_s:
                    notification.notify(
                        title="Serveur",
                        message="Arrêt du serveur",  # Message de notification.
                        app_name='My Application',
                        app_icon=None,  # Optionnel : chemin vers une icône.
                        timeout=50  # Durée avant la disparition de la notification.
                    )
                ...
                Clock.unschedule(self.event_check)

        def check_thread():
            # Démarre l'intervalle pour vérifier les clients
            self.event_check = Clock.schedule_interval(check_for_clients, 1)  # Vérifie toutes les secondes
    ...
    def update_clients(self):
        ...
        for user in liste:  # Parcourt chaque utilisateur
            user_name, desc, u_pub = liste[user].split(':')  # Sépare les informations de l'utilisateur
    ...
    def verification(self):
        ...
        def check_for_messages(dt):
            ...
            if self.verifier:
                ...
                Clock.unschedule(self.event_verifier)


        def verification_thread():
            # Démarre une vérification régulière des messages.
            self.event_verifier = Clock.schedule_interval(check_for_messages, 1)  # Vérifie toutes les secondes.
    ...
    def notification(self, msg):
        ...
        elif command.strip() == "refusé":
            msg_n = content + " a " + command

        elif command.strip() == "retiré":
            msg_n = content + " vous a " + command

        # Affiche une notification pour confirmer l'état de la discussion.
        notification.notify(
            title="confirmation de discussion",
            message=msg_n,  # Message de notification indiquant l'état de la discussion.
            app_name='My Application',
            app_icon=None,  # Optionnel : chemin vers une icône.
            timeout=50  # Durée avant la disparition de la notification.
        )
