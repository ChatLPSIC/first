...
class ServerHandler:
    ...
    def remove_client(self, user):
        if user in self.cli_pseudo:
            if self.cli_pseudo[user] in self.clients:
                self.clients.remove(self.cli_pseudo[user])
        if user in self.cli_pseudo:
            del self.cli_pseudo[user]
        if user in self.pseudo_ip:
            del self.pseudo_ip[user]
    ...
    def process_message(self, message, client_socket):
        ...
        if command == "info_p":  # Commande pour définir le pseudonyme
            ...
            if user not in self.cli_pseudo:  # Si le pseudonyme n'est pas déjà associé
                ...
                self.pseudo_ip[user] = content  # Associe le pseudonyme à l'adresse IP
                ...
                try:
                    self.envoyer(user, "connexion:deja")
                    #time.sleep()
                    if client_socket in self.clients:
                        self.clients.remove(client_socket)
                    client_socket.close()  # Ferme le socket en cas d'erreur
                except Exception as e:
                    print(f"Failed to connect: {e}")
                    client_socket.close()  # Ferme le socket en cas d'erreur
        ...
        elif command == "deconnexion":  # Commande pour envoyer une réponse
            self.broadcast(command + content)
            #time.sleep(0.5)
            self.remove_client(content)
...
class ClientHandler:
    ...
    def close_socket(self):
        ...
        self.send_message("deconnexion:" + self.pseudo)
        time.sleep(1)
        if self.connected:
            try:
